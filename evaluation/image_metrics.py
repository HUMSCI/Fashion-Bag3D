...
class GramLoss(nn.Module):
    ...
    def __init__(self, device=None):
        ...
        self.model = models.vgg19(pretrained=True).features.to(device).eval()
        self.model = self.model[:-2]  # Remove the classifier part (fully connected layers)
    ...
    def forward(self, input, target):
        """
        Computes the style loss between the input (generated) and target (reference) images.
        """
        src_feats = self.get_feats(self.normalize(input))  # Extract features from the input image
        target_feats = self.get_feats(self.normalize(target))  # Extract features from the target image
        
        init_loss = 0.
        # Compute the style loss for each layer's feature map
        for idx in range(len(src_feats)):
            loss = F.mse_loss(self.gram_matrix(src_feats[idx]), self.gram_matrix(target_feats[idx]))
            init_loss += loss
        
        # Normalize the total loss by the number of layers used
        init_loss /= len(src_feats)
        return init_loss
